...
class Twitch(commands.Cog):
    ...
    @commands.slash_command(description="Adds a streamer to your watchlist")
    @commands.has_permissions(administrator=True)
    async def add(self, inter: disnake.ApplicationCommandInteraction, streamer):
        try:
            ...
            if streamer in data[str(inter.guild.id)]["watchlist"]:
            ...
            elif streamer not in data[str(inter.guild.id)]["watchlist"]:
                data[str(inter.guild.id)]["watchlist"].append(streamer.lower())
    ...
    @commands.slash_command(description="Removes a streamer from your watchlist")
    @commands.has_permissions(administrator=True)
    async def remove(self, inter: disnake.ApplicationCommandInteraction, streamer):
        try:
            with open('json/guild.json', 'r', encoding='UTF-8') as file:
                data = json.load(file)

            if streamer not in data[str(inter.guild.id)]["watchlist"]:
                alreday_streamer_error_embed = disnake.Embed(
            ...
            elif streamer in data[str(inter.guild.id)]["watchlist"]:
                data[str(inter.guild.id)]["watchlist"].remove(streamer.lower())
                with open('json/guild.json', 'w', encoding='UTF-8') as dump_file:
                    json.dump(data, dump_file, indent=4)
    ...
    @commands.slash_command(description='Checks the twitch stream watchlist from this server')
    async def check(self, inter: disnake.ApplicationCommandInteraction):
        ...
        users = get_users(guild_data[str(inter.guild.id)]["watchlist"])
        ...
        if len(streams) == 1:
            await inter.response.send_message(
                f"{inter.author.mention} Dein Stream Check. Es ist **1 Streamer Live!**",
                embed=embed
            )
        else:
            await inter.response.send_message(
                f"{inter.author.mention} Dein Stream Check. Es sind insgesamt **{len(streams)} Streamer Live!**",
                embed=embed
            )
