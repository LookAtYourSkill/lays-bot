...
class Roles(commands.Cog):
    ...
    async def role(self, inter: disnake.ApplicationCommandInteraction, member: disnake.Member, role: disnake.Role, reason='Role remove/add per Command'):
        if inter.author.top_role.position > role.position:
            if role in member.roles:
                ...
                top_role_under_role_embed = disnake.Embed(
                    description=f'Deine Höchste Rolle [`{inter.author.top_role}`] ist niedriger als die Rolle [`{role}`], die du hinzufügen möchtest',
                    color=disnake.Color.red()
                )
                await inter.response.send_message(
        ...
        elif role not in inter.guild.roles:
